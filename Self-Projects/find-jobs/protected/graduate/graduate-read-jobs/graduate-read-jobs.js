  ...
  jobContainer.onclick = async function (event) {
    if (event.target.matches(".j")) {
      ...
      <div class="apply-job-container">
        <button class="apply-job btn btn-primary" data-id="${targetId}">Apply</button>
      </div>
      ...
      <ul class="list-group list-group-flush job jo" data-id="${targetId}">
        ...
        <li class="list-group-item jo bd" data-id="${targetId}"> Details : ${result.details}</li>
          ...
          `;
      document.querySelector(".apply-job").onclick = async function (event) {
        const targetId = event.target.getAttribute("data-id");
        console.log(targetId);
        ...
        loadJobs();
        window.alert("Successfully applied, Let's Rock!!")
      };
    }
  };
}
...
socket.on("new-job", function (job) {
  ...
  jobContainer.innerHTML += `
            <div class="job j" data-id="${job.id}">
                <div class="j" data-id="${job.id}">${job.position}</div>
                <div class="j" data-id="${job.id}">${job.company}</div>
                <div class="j" data-id="${job.id}">${job.location}</div>
                <div class="j" data-id="${job.id}">${job.created_at.substring(0,10)}</div>
            </div>`;
  loadJobs();
});
